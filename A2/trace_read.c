...
bool trace_match_next(trace_p tracer, val address, val value) {
    ...
    if (!tracer->next_valid) {
        printf(" -- end of tracefile reached, while trying to match %lx <- %lx\n", address.val, value.val);
        return false;
    }
    if (tracer->next_addr != address.val) {
        printf(" -- mismatch, writing to address %lx, tracefile expected write to %lx\n", address.val, tracer->next_addr);
        return false;
    }
    if (tracer->next_value != value.val) {
        printf(" -- mismatch while writing to address %lx\n", address.val);
        printf(" -- writing value %lx, tracefile expected %lx\n", value.val, tracer->next_value);
        return false;
    }
    /* matched! */
