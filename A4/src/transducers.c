...
int transducers_link_1(stream **out,
  ...
  if (in -> flag == 0) {
    ...
    //Child process which works on the input
    if (ret == 0) {
      in -> flag = 1;
      assert(fclose(files[0])==0);
    ...
    //Parent process which works on the output
    else {
      assert(fclose(files[1])==0);
...
// Transducer used to work between 2 input streams and 1 output stream
...
int transducers_dup(stream **out1,stream **out2, stream *in) {
  ...
  if (in -> flag == 0){
    ...
    //Doing everything twice, once for every output stream
